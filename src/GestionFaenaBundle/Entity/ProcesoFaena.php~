...
abstract class ProcesoFaena
{
    ...
    */      
    ...
    public function existeArticuloDefinidoManejoStock(\GestionFaenaBundle\Entity\gestionBD\Articulo $articulo, $idArticulo = null)
    {
        foreach ($this->manejosStock as $stock) 
        {
            $idArt = $stock->getArticulo()->getId();
            if (($idArticulo) && ($idArt == $idArticulo))
            {
                return $stock;
            }
            elseif ($idArt == $articulo->getId())
            {
                return $stock;
            }
        }
